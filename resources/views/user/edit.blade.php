    ...
    <div class="card-body">
      <form action="{{ route('user.update', $item->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <input type="number" name="phone" value="{{ $item->profile->phone ?? '' }}" required class="form-control" id="basic-default-name" placeholder="" />
          </div>
        </div>
        <div class="row mb-3">
          <label class="col-sm-2 col-form-label" for="basic-default-name">Alamat</label>
          <div class="col-sm-10">
            <input type="text" name="alamat" value="{{ $item->profile->alamat ?? '' }}" required class="form-control" id="basic-default-name" placeholder="" />
          </div>
        </div>
        <div class="row mb-3">
